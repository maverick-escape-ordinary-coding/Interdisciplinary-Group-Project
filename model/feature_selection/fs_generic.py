...
def feature_selection(config_file_name):
    ...
    for alpha in alphas:
        ...
        sfm = SelectFromModel(lasso, threshold=0.25, norm_order=1, max_features=None)
        sfm.fit(X_train, y_train)
        feature_idx = sfm.get_support()
        feature_name = X_train.columns[feature_idx]
        print('FEATURES AMOUNT: ', len(feature_name))
        print("Lasso train/test scores for alpha={} {:.2f}/{:.2f}".format(alpha, train_score, test_score))
        print("Coefs used={:.2f}".format(coeff_used))
        feat_acc_dict[round(test_score, 3)] = feature_name
    ...
    for alpha in alphas:
        ...
        sfm = SelectFromModel(ridge, threshold=0.25, norm_order=1, max_features=None)
        sfm.fit(X_train, y_train)
        feature_idx = sfm.get_support()
        feature_name = X_train.columns[feature_idx]
        print('FEATURES AMOUNT: ', len(feature_name))
    ...
    plt.ylabel('Coefficient Magnitude',fontsize=16)
    plt.legend(fontsize=13, loc=2)
    plt.savefig('Ridge_features.png', bbox_inches='tight')
    plt.show()

    #%% feature selection df
    max_acc = max(feat_acc_dict.keys())
    selected_feats = feat_acc_dict[max_acc]
    ...
    # return X[X.columns.intersection(selected_feats)], y
